...
public class SpeechSandboxStreaming : MonoBehaviour
{
    ...
    private string stt_username = "1a4df691-e25a-4e2d-99ff-d33fc8140541";
    private string stt_password = "dKxlUiiDsapl";
	//PC endpoint: comment it out and uncomment the android endpoint before deploying to android
    private string stt_url = "https://stream.watsonplatform.net/speech-to-text/api";
	//Android endpoint: uncomment the android endpoint and comment the PC endpoint out before deploying to PC
	//private string stt_url ="https://stream-tls10.watsonplatform.net/speech-to-text/api";

    private string convo_username = "39a36beb-c7b9-4dfb-a0bd-914881213c21";
    private string convo_password = "EE8brEGlSMUC";
	//PC endpoint: comment it out and uncomment the android endpoint before deploying to android
    ...
    private int score;
    public Text CanNotHearYouFlashMessage;
    public Canvas ErrorCanvasObject; // Assign in inspector
    ...
    private string convo_workspaceId = "e67087fd-c779-4608-a21c-5e8b0924130b";
    ...
    void OnMessage(object resp, Dictionary<string, object> customData)
    {
        ...
        if (resp != null && (messageResponse.intents.Length > 0 || messageResponse.entities.Length > 0))
        {
            ...
            {
				//SceneManager.LoadScene ("Autism1");
				//_animator.SetBool("isSorry", true);
				//Debug.Log ("He is playing again");
            }
            else if (intent == "help")
            {
            ...
            {
                //Turn 180 degree
				//transform.Rotate (new Vector3 (0.0f, 180));

				//if (!hasTurned) {
				//	container.Rotate (new Vector3 (0, 180, 0));
				//	hasTurned = true;
				//}
				//_animator.SetBool("isTurn", true);
                _animator.SetTrigger("isTurn");
                //_containerAnimator.SetTrigger ("rotate");

                //Trigger greeting animation after 3 seconds
                //Invoke("TriggerGreetingsAnimation", 3f);
			...
			{
                //_animator.SetBool("isIgnore", true);
                //transform.Rotate (new Vector3 (0.0f, 180));
                //_animator.SetBool("isIgnore", true);
                ...
                //    hasTurned = true;
                //}
                _animator.SetTrigger("isIgnoor");
                _animator.SetBool("isFacebook", true);

                //Check that score is not empty before substracting
                if (score >= 5)
				{
					//Remove 2 From scored Points
					score -= 5;
					UpdateScore();
				}
				Debug.Log ("He is back to playing");
			}
			else if (intent == "Good")
			{
				//transform.Rotate (new Vector3 (0.0f, 180));
				...
				//	hasTurned = true;
				//}
                //Triger mixamo animation with tunde introducing himself

                Invoke("TriggerGreetingsAnimation", 1f);
        ...
        {
            //Show can't hear you error
            //StartCoroutine(ShowMessage("Abc", 2));
            ErrorCanvasObject.enabled = ErrorCanvasObject.enabled;
    ...
    private void TriggerGreetingsAnimation()
    {
        _animator.SetTrigger("isGreeting");
    }

    //IEnumerator ShowMessage(string message, float delay)
    //{
    //    CanNotHearYouFlashMessage.text = message;
    //    CanNotHearYouFlashMessage.enabled = true;
    //    yield return new WaitForSeconds(delay);
    //    CanNotHearYouFlashMessage.enabled = false;
    //}
